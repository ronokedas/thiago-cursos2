      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {mediaTab === 'PRACTICAL' && <section className="rounded-2xl border border-neutral-800 bg-neutral-900/40 p-4 space-y-3">
            ...
            {!lessonData.progress.mainVideoEndedAt ? <p className="text-xs text-neutral-400">Finalize o vídeo principal para liberar os vídeos curtos.</p> : lessonData.lesson.practicalVideos?.length ? <div className="grid gap-2 sm:grid-cols-2">{lessonData.lesson.practicalVideos.map(video => {
              const isPlaying = practicalStream?.id === video.id;
              return <button type="button" key={video.id} onClick={() => selectPracticalVideo(video)} aria-pressed={isPlaying} className={`rounded-xl border p-3 text-left transition-colors ${isPlaying ? 'border-amber-400 bg-amber-500/10 shadow-sm shadow-amber-500/10' : 'border-neutral-800 bg-neutral-950 hover:border-amber-500/50'}`}><div className="flex items-center justify-between gap-2"><p className={`truncate text-xs font-bold ${isPlaying ? 'text-amber-300' : 'text-neutral-100'}`}>{video.title}</p>{isPlaying && <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-amber-500 px-2 py-0.5 text-[9px] font-bold text-neutral-950"><Play className="h-3 w-3 fill-current" />Assistindo agora</span>}</div><p className={`mt-1 text-[11px] line-clamp-2 ${isPlaying ? 'text-amber-100/70' : 'text-neutral-500'}`}>{video.description || 'Vídeo complementar de operação.'}</p></button>;
            })}</div> : <p className="text-xs text-neutral-500">Ainda não há vídeos práticos nesta aula.</p>}
          </section>}
